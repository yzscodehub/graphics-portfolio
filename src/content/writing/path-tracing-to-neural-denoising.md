...
ONNX Runtime Web 可以按能力选择 WebGPU 或 WASM。当前页面按 `WebGPU → WASM → 确定性 Canvas 近似` 回退，并只把实际模型执行区间报告为推理耗时。WASM 单线程更容易部署，但速度依设备变化；WebGPU 可能因驱动或浏览器限制初始化失败。

失败处理必须是可见的：模型下载失败、输入形状不符、执行后端不可用时，页面显示原因并使用明确标注的确定性 Canvas 近似。访问者仍能阅读方法和限制，不会被一个 GPU 错误挡在项目之外；真实预计算验证图集保留为后续路线。
